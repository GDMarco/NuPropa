...
\end{align}
%
Making use of Eq.~\eqref{eq:dsigma}, the total cross-section is
\begin{align}
\sigma_{\nu_1 \nu_1\to \nu_1 \nu_1}(s_{12}) &= \\
								&\approx
\end{align}
%
In the second line we have provided an approximation of the result valid for $s_{12} \gtrsim m_Z^2$ which is both reliable and numerically stable in such a limit.
...
Following the logic of the previous discussion, it also possible to consider the impact of $\mathcal{O}(\alpha^4)$ that involve contribution from two resonantly enhanced massive gauge boson propagators.
%
